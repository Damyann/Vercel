  ...
  try {
    ...
    function parseDate(str) {
      if (str.includes('/')) {
        ...
        return DateTime.fromObject({ year: y, month: m, day: d }).plus({ hours: 2 }).toJSDate();  // Добавяме 2 часа към датата
    ...
    const now = DateTime.now().plus({ hours: 2 }).toJSDate();  // Използваме Софийско време (UTC+2)
